<template>
  <div class="shadow h-screen">
    ...
    <el-form label-position="top" class="pb-4 px-4 pt-2">
      <input
        type="text"
        class="mb-4 w-full min-h-min max-h-max text-2xl rounded focus:outline-0"
        v-model="task.title"
...
function removeAttachment(attachmentId: number) {
  destroy(url(`tasks/${props.task.id}/attachments/${attachmentId}`)).then(
    () => {
      const index = props.task.attachments.findIndex(
...
function removeFollower(followerId: number) {
  destroy(url(`tasks/${props.task.id}/followers/${followerId}`)).then(() => {
    const index = props.task.followers.findIndex(
